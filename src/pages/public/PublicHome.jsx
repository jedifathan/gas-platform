...
export default function PublicHome() {
  ...
  return (
    <div>
      ...
      <section className="bg-gradient-to-br from-primary-950 via-primary-800 to-primary-700 text-white py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black leading-tight mb-5 text-balance">
            Gigi Anak Sehat,<br />
            <span className="text-primary-300">Masa Depan Cerah</span>
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            <button onClick={() => navigate('/login')}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary-500
                         hover:bg-primary-400 text-white font-semibold text-sm transition-colors">
              Masuk ke Platform <ArrowRight size={16} />
            </button>
            <Link to="/about"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-primary-400/40
                         text-primary-200 hover:bg-primary-400/10 font-medium text-sm transition-colors">
              Tentang Program
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {FEATURES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="card p-5 flex gap-4 hover:border-primary-200 transition-colors">
              <div className="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center shrink-0">
                <Icon size={18} className="text-primary-600" />
              </div>
              <div>
      ...
      <section className="bg-alabaster border-y border-gray-100 py-14 px-6">
              ...
              {top3.map((school, i) => {
                ...
                return (
                  <div key={school.school_id}
                    className="card p-4 flex items-center gap-4 hover:border-primary-200 transition-colors">
                    ...
                    <span className="text-lg font-black text-primary-700 shrink-0">{school.total_score}</span>
                  </div>
                )
              })}
            </div>
      ...
      <section className="max-w-3xl mx-auto px-6 py-16 text-center">
        ...
        <button onClick={() => navigate('/login')}
          className="inline-flex items-center gap-2 px-7 py-3 rounded-xl bg-primary-600
                     hover:bg-primary-700 text-white font-semibold text-sm transition-colors">

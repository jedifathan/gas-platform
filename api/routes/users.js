...
router.post('/', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const id   = uid()
    const hash = await bcrypt.hash(password, 12)   // ← bcrypt hash

    await pool.query(
      `INSERT INTO users (id, name, email, password, role, school_id, region_id, is_active)
       VALUES ($1,$2,$3,$4,$5,$6,$7,true)`,
      [id, name.trim(), email.trim().toLowerCase(), hash, role, school_id || null, region_id || null],
    )
...
router.patch('/:id', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE users SET name=$1, email=$2, password=$3, role=$4, school_id=$5, region_id=$6 WHERE id=$7`,
      [name, email.toLowerCase(), passwordHash, role, school_id, region_id, id],
    )
...
router.patch('/:id/toggle', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return res.json({ success: false, error: 'USER_NOT_FOUND' })
